...
# print("2. Get new access token-------------------")
# session = requests.session()
# session.verify = "ca-bundle.crt"

# base_url = 'https://aimlms.muc-gp.icp.infineon.com'
# project_key= 'AIMLTST2'
# path_to_cert = "ca-bundle.crt"
...
# }
# response = session.post(f'{base_url}/v1/oauth/token', data=data, verify=path_to_cert)
...
response = session.get(f'{base_url}/v1/projects/{project_key}/artifacts/metadata')
project_list = json.loads(response.text)
print('Response Received: ' + str(response.status_code) + ': ' + json.dumps(project_list, indent=4))

with open('list_artifacts.json', 'w') as f:
